...
/* setting initial screen of camera */
...
/* extract rgb values of a current screen taken by the camera */
...
/* main function showing real time video taken by the camera */
...
void* cameraview(void* flag)		
{
	...
	while(1)
	{
		/* if flag= 0  then end camera and thread */
		 if(*((int*)flag) == 0)
		{
			stopPreview();
			DestroyCamera();
...
/* initiate making file settingfile */
...
/* save the screen taken by the camera current in buffer */
...
/* create image file named picture.img */
...
int screenshot()
{
	...
	/* insert current screen into buffer and create a image file */
